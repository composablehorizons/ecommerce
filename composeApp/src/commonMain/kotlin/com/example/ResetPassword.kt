    ...
    Column(horizontalAlignment = Alignment.CenterHorizontally, modifier = Modifier.fillMaxSize().background(LocalBackgroundColor.current)) {
        CompositionLocalProvider(LocalContentColor provides LocalOnBackgroundColor.current) {
            Column(verticalArrangement = Arrangement.spacedBy(12.dp), modifier = Modifier.widthIn(max = 600.dp).fillMaxWidth().weight(weight = 1f, fill = false).fillMaxHeight().verticalScroll(rememberScrollState()).padding(16.dp)) {
                Text("Reset Password", modifier = Modifier.fillMaxWidth(), style = LocalTitleLargeTextStyle.current, fontSize = 16.sp)
                Text("Enter the new password for your account", modifier = Modifier.fillMaxWidth().alpha(0.66f), style = LocalBodyLargeTextStyle.current, fontSize = 16.sp)
                Spacer(Modifier.height(12.dp).fillMaxWidth())
                Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
                    Text("New password", modifier = Modifier.fillMaxWidth(), style = LocalTitleMediumTextStyle.current, fontSize = 16.sp)
                    TextField(value = "", onValueChange = { /* TODO Handle this */ }, modifier = Modifier.fillMaxWidth(), maxLines = 1, placeholderText = "A strong password", contentPadding = PaddingValues(start = 8.dp, top = 12.dp, end = 8.dp, bottom = 12.dp), borderWidth = 1.dp, shape = RoundedCornerShape(LocalCornerRadius.current), textStyle = LocalBodyLargeTextStyle.current, fontSize = 16.sp)
                }
                Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
                    Text("Re-type your new password", modifier = Modifier.fillMaxWidth(), style = LocalTitleMediumTextStyle.current, fontSize = 16.sp)
                    TextField(value = "", onValueChange = { /* TODO Handle this */ }, modifier = Modifier.fillMaxWidth(), maxLines = 1, placeholderText = "A strong password", contentPadding = PaddingValues(start = 8.dp, top = 12.dp, end = 8.dp, bottom = 12.dp), borderWidth = 1.dp, shape = RoundedCornerShape(LocalCornerRadius.current), textStyle = LocalBodyLargeTextStyle.current, fontSize = 16.sp)
                }
                ...
                PrimaryButton(onClick = { /* TODO Handle this */ }, contentPadding = PaddingValues(start = 8.dp, top = 12.dp, end = 8.dp, bottom = 12.dp), modifier = Modifier.fillMaxWidth()) {
                    Text("Continue", modifier = Modifier.fillMaxWidth(), style = LocalBodyLargeTextStyle.current, textAlign = TextAlign.Center, fontSize = 16.sp)
                }
            }
        }
    }
